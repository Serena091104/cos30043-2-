  ...
  methods: {
    ...
    async likeProduct(productId) {
      ...
      try {
        ...
        const data = await response.json();
        if (data.success) {
          this.likes[productId] = (this.likes[productId] || 0) + 1;
        } else {
          console.error('Error liking product:', data.error);
        }
      } catch (error) {
    ...
    async fetchComments(productId) {
      try {
        ...
        if (Array.isArray(data)) {
          this.$set(this.comments, productId, data);
        }
      } catch (error) {
